...
| `comments` | `legal` | keep: `legal`, `none`, `all` |
...
oxc's codegen treats `minify` as a whitespace setting, so comments survive it
and have to be removed separately. The default here drops ordinary and jsdoc
comments but keeps legal ones (`@license`, `@preserve`), because silently
deleting a licence header is not something a build tool should do on your
behalf. `oxc-minify` drops those too, so pass `comments: "none"` if you want
output identical to it.

## Source map fidelity

When you pass an input map, the composed map resolves each position through it
to the *original* source, and drops mappings that resolve only to the
intermediate bundle -- a build artifact a debugger cannot fetch. That makes the
map smaller (about half, on a large real application, since the intermediate's
`sourcesContent` is no longer carried) without losing anything you can open.

Columns are token-granularity: each position maps to the start of the input
map's enclosing token. `webpack-sources` is finer, because it emits a mapping
per identifier by scanning the source text. The gap is only visible on a line
the bundler copied verbatim, and closing it by arithmetic does not work -- the
offset is meaningless on a transformed line and produces columns past the end
of the line they name.
